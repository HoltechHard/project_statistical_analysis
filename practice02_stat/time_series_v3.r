...
# take the best model using criteria AIC and BIC => take min values
ids <- NULL
aic_values <- NULL
bic_values <- NULL
...
for(j in 1:length(func)){
  ids <- c(ids, j)
  model <- lme(fixed = form[[j]], random =~ 1|ID,
                data = datavar9, method = "ML")
  aic_values <- c(aic_values, AIC(model))
  bic_values <- c(bic_values, BIC(model))
}

# build the information-criteria table which contains AIC and BIC values
IC_table <- data.frame(id = ids, model = func,
                AIC = aic_values, BIC = bic_values)
print(IC_table)

# take the best model for AIC criteria
best_modelAIC <- IC_table[IC_table$AIC == min(IC_table$AIC), ]
print(best_modelAIC)

# take the best model for BIC criteria
best_modelBIC <- IC_table[IC_table$BIC == min(IC_table$BIC), ]
print(best_modelBIC)

# check the parameters inside of the optimal model
opt_id <- IC_table[IC_table$AIC == min(IC_table$AIC), ]$id
print(paste("opt-id = ", opt_id))
form_opt <- form[opt_id]
model_opt <- lme(fixed = form[[opt_id]],
                    random =~ 1|ID, data = datavar9, method = "ML")
print(model_opt)

# check the significance of the parameters in the optimal model
anova_opt <- anova(model_opt)
print(anova_opt)

# optimal formula
print(form_opt)

# study of ausence in influence of factor A
id_m9A <- as.numeric(which(func == "Y ~ B*Visit + B*Visit2"))
model_m9A <- lme(fixed = form[[id_m9A]], random =~ 1|ID,
                    data = datavar9, method = "ML")
anova_m9A <- anova(model_m9A, model_opt)
print(anova_m9A)
print(anova_m9A$"p-value"[2])   # reject HA

# study of ausence in influence of factor B
id_m9B <- as.numeric(which(func == "Y ~ A + Visit + Visit2"))
model_m9B <- lme(fixed = form[[id_m9B]], random =~ 1|ID,
                    data = datavar9, method = "ML")
anova_m9B <- anova(model_m9B, model_opt)
print(anova_m9B)
print(anova_m9B$"p-value"[2])   # reject HB

# study of ausence in influence of time
model_m9T <- lme(fixed = Y ~ as.factor(A) * as.factor(B), random =~ 1|ID,
                    data = datavar9, method = "ML")
anova_m9T <- anova(model_m9T, model_opt)
print(anova_m9T)
print(anova_m9T$"p-value"[2])   # reject HT

# compare results of AIC and cAIC for best model
aic_best <- AIC(model_opt)
print(aic_best)

caic_best <- cAIC(model_opt)
print(caic_best$caic)
...
# take partial the confidence interval for the set of model parameters
alpha <- 0.05
print(func[opt_id])
q10 <- lme(fixed = form[[opt_id]], random =~ 1|ID,
                  data = datavar9, method = "ML")
intervals(q10, which = "fixed", level = 1 - alpha)

# take generic confidence interval for the set of model parameters
alpha2 <- alpha / length(q10$coefficients[[1]])
print(paste("alpha for bonferroni = ", alpha2))
intervals(q10, which = "fixed", level = 1 - alpha2)
